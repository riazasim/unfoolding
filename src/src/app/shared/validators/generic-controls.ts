...
import * as ErrorMessages from './errors.keys';
import {
...
function createControl<T>(required: boolean): IFormControl<T> {
  return required ? createRequiredControl<T>() : new FormControl(null);
}
...
export function createDisabledFormControl<T>(initialValue: T | null = null): IFormControl<T> {
  return new FormControl({ value: initialValue, disabled: true });
}
